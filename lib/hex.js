...
HexGrid.prototype.checkPoint = function (x, y, done) {
	...
	x = x - grid.sx + this.radius;
	y = y - grid.sy + this.radius;
	
    console.log(x / (grid.radius / 2));
    ...
    if (x > 0 & y > 0 & x < this.radius * 2 * (this.w - 1) & y < this.hexH * (this.h + 1)) {
        // if a triangle box
        if (tri) {
            ...
            index = this.find(x + grid.sx - this.radius, y + grid.sx -this.radius);
