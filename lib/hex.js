...
var HexGrid = function (sx, sy, w, h, radius) {
    ...
    hexH = Math.sqrt(3) / 2 * size; // hex height ( I found that "Math.sin(Math.PI / 3) * size" also works )
    ...
    while (x < w) {
        ...
        while (y < h) {
            ...
            this.hex.push(new Hex(
                    Math.round(sx + hexW * x),
                    Math.round(sy + hexH * y + hexH / 2 * willOffset),
                    radius));
...
HexGrid.prototype.checkPoint = function (x, y, done) {
    ...
    console.log('checking a point at : (' + x + ',' + y + ')');
